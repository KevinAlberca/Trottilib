...
if (isset($_SESSION['user'])) {
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-offset-3 col-sm-6 text-center">
            <form class="form-horizontal" method="POST" name="registration_form">
                ...
                <div class="col-xs-12">
                    <input type="checkbox" name="C.G.U" id="C.G.U" value="checked" /> <label for="C.G.U">Acceptez-vous nos Conditions Générale d'Utilisation ?</label>
                </div>
            <div class="form-group col-xs-12">
        ...
        if(isset($_POST) && !empty($_POST)){
            if($_POST['C.G.U'] == 'checked'){
                if($pwd === $pwdCheck){
                    if($user->addUser($nom, $prenom, $email, $date_naissance, $pwd, $adresse, $code_postal, $ville)){
                        $user->setDataSearch($nom, $prenom, $email, $ville);
                        echo "<p class='successBlock'>Le compte a été créé</p><script>setTimeout(function(){document.location.href='./?page=connexion';}, 2500);</script>";
                    } else {
